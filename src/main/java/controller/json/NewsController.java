...
public class NewsController {
  ...
  public Object kblToday() throws Exception{
    ...
    try {
      return JsonResult.success(getNews());
    } catch (Exception e) {
  ...
  public Object getNews() throws Exception {
    Document document = Jsoup.connect("http://sports.news.naver.com/basketball/news/index.nhn").get();
    Elements currNews = document.select("#rankingNewsList_0 > li:lt(1)");
    String newsTitle = currNews.select("span").text();
    String newsSrc = currNews.select("a").attr("href");
    Elements currVideo = document.select(".aside_vod_list2 > li:lt(1)");
    
    String videoSrc = currVideo.select("a").attr("href");
    String videoTitle = currVideo.select(".title > span").text();
    String videoImage = currVideo.select("img").attr("lazy-src").split("src=")[1];
    
    HashMap<String, Object> map = new HashMap<>();
    map.put("newsTitle", newsTitle);
    map.put("newsSrc", newsSrc);
